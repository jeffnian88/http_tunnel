...
char request_connection_open[]=
"GET http://%s:%s/ HTTP/1.1\r\n"
"Host: %s:%s\r\n"
"My-Request: connection_open\r\n"
"Pragma: no-cache\r\n"
"Proxy-Connection: Keep-Alive\r\n"
"Proxy-Authorization: Basic amgubmlhbjpzdTNjbDM=\r\n"
"\r\n";
char request_connection_close[]=
"GET http://%s:%s/ HTTP/1.1\r\n"
"Host: %s:%s\r\n"
...
"Proxy-Authorization: Basic amgubmlhbjpzdTNjbDM=\r\n"
"\r\n";
char request_connection_recv_data[]=
"GET http://%s:%s/ HTTP/1.1\r\n"
"Host: %s:%s\r\n"
...
"Proxy-Authorization: Basic amgubmlhbjpzdTNjbDM=\r\n"
"\r\n";
char request_connection_send_data[]=
...
"Proxy-Authorization: Basic amgubmlhbjpzdTNjbDM=\r\n"
"\r\n";
char request_connection_post_data[]=
"POST http://%s:%s/ HTTP/1.1\r\n"
"Host: %s:%s\r\n"
"My-Request: connection_send_data\r\n"
"Content-Length: %d\r\n"
...
int Wrapper_Request_Open_Server_Connection(int* proxy_server_socket)
{
    if(*proxy_server_socket == -1)
    {
        ...
        if((*proxy_server_socket = socket(AF_INET, SOCK_STREAM, 0)) == ERROR)
        {
    ...
    // build request
    char request[10240];
    sprintf(request, request_connection_open, remote_proxy_ip, remote_proxy_port, remote_proxy_ip, remote_proxy_port);
...
// @return -1 : fail
*/
int Wrapper_Request_Open_Recv_Data_Connection(int* recv_data_socket)
{
    // recv_data_socket is not valid
    if(*recv_data_socket == -1)
    {
        // create recv_data_socket
        if((*recv_data_socket = socket(AF_INET, SOCK_STREAM, 0)) == ERROR)
        {
    ...
    // build request
    char request[10240];
    sprintf(request, request_connection_recv_data, remote_proxy_ip, remote_proxy_port, remote_proxy_ip, remote_proxy_port, reopen++);
...
// @return -1 : fail
*/
int Wrapper_Request_Open_Send_Data_Connection(int* send_data_socket)
{
    // send_data_socket is not valid
    if(*send_data_socket == -1)
    {
        // create send_data_socket
        if((*send_data_socket = socket(AF_INET, SOCK_STREAM, 0)) == ERROR)
        {
    ...
    // build request
    char request[40960];
    sprintf(request, request_connection_send_data, remote_proxy_ip, remote_proxy_port, remote_proxy_ip, remote_proxy_port, resend++, gsend_Content_Length);
